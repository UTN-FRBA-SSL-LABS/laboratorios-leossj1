...
> R:Inicializa la estructura del carrito, es fundamental llamarla antes porque evita valores basura y previene comportamiento indefinido.
...
> R:LINEA 24, suma al total solamente el precio de los items.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Multiplicar el precio de los items por la cantidad de items.
...
> R:Sí, las líneas 29 y 30 pertenecientes a la función carrito_descuento. No se ejecutaron porque el ejecutable de cobertura se compiló únicamente con tests/test_unitarios.c, donde nunca se invoca a carrito_descuento (dicha función solo se prueba en los tests de integración).
...
> R:El unitario prueba una función aislada; el de integración prueba varias funciones interactuando entre sí. El bug lo detectó primero el test unitario (test_total_con_cantidad).
...
> R:En C no hay control de límites de memoria (bounds checking), lo que causa corrupción de datos o caídas (stack smashing). En Python/Java se lanza automáticamente una excepción de índice fuera de rango (IndexError / ArrayIndexOutOfBoundsException).
...
> R:Es automatizado, reproducible, evita asumir cosas por sesgo visual y previene regresiones al modificar el código.
...
> R:Porque multiplicar por 1 da el mismo resultado que no multiplicar ($350x1 = 350$), enmascarando el error.
